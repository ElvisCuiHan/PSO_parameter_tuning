...
def Elastic_pso(b, X, y, cv, task):
    ...
    for i in range(n_particles):
        ...
        if task == "Regression":
            scores = cross_val_score(elastic_net, X, y, cv=kf, scoring='neg_mean_absolute_error') +\
                cross_val_score(elastic_net, X, y, cv=kf, scoring='neg_mean_squared_error')
...
def Lasso_pso(b, X, y, cv, task):
    ...
    for i in range(n_particles):
        ...
        if task == "Regression":
            scores = cross_val_score(lasso_reg, X, y, cv=kf, scoring='neg_mean_absolute_error')
            cost[i] = -np.mean(scores)
        else:
